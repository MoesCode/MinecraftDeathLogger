...
def getDeathMessages():
	f = open(langLocation)								#open language file
...
def scanForDeath(message):
	...
	for reason in allDeathMessages:			#for every reason
		...
		if match != None:							#if there is one
			return match							#return that match
...
def getNewDeaths():
	...
	for d in newDeaths:				#for every new death
		actionOnDeath(d)				#calls actionOnDeath and possible actions added by user
		

def process():
	global checkInterval				#reference global var
	global shouldRun					#reference global var
	while shouldRun:					#noexplain
...
if __name__ == '__main__':												#if the program has been started as standalone
	...
	while shouldRun:														#repeat during runtime
		...
		if s == 'stop':													#if input was 'stop'
			...
			sys.exit()														#finally quit the program
